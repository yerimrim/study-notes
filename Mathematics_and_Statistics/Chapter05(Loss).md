...
- **Maximum Likelihood**: $\arg\max \sum_i P[y_i \mid f(x_i, \phi)]$
- **Maximum Log Likelihood**: $\arg\max \sum_i \log(P[y_i \mid f(x_i, \phi)])$
- **Minimum Negative Log Likelihood**: $\arg\min \left( - \sum_i \log(P[y_i \mid f(x_i, \phi)]) \right)$
