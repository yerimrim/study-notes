...
$$
a_1x + b_1y + c_1z = d_1\\a_2x+b_2y+c_2z=d_2\\a_3x+b_3y+c_3z=d_3\\ x^{(0)} = k_1,\space yx^{(0)} = k_2, \space z^{(0)} = k_3 \\ \epsilon_a=n\%
$$
...
$$
x = \frac {d-by-cz}{a}\\y = \frac {d-ax-cz}{b} \\ z = \frac {d-ax-by}{c}
$$

**Step3) Calculate   $x^{(n+1)},\space  y^{(n+1)}, \space z^{(n+1)}$**

**1. Jacobi Method**

$$
x^{(k)} = \frac {d-by^{(k-1)}-cz^{(k-1)}}{a}\\y^{(k)} = \frac {d-ax^{(k-1)}-cz^{(k-1)}}{b} \\ z^{(k)} = \frac {d-ax^{(k-1)}-by^{(k-1)}}{c}
$$
